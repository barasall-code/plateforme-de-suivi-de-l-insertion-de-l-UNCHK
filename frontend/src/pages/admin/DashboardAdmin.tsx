...
interface MoisData { mois: number; nombre: number; }
interface FiliereData { filiere: string; nombre: number; }
interface NiveauData { niveau: string; nombre: number; }
interface TypeOffreData { type: string; nombre: number; }
interface StatutData { statut: string; nombre: number; }
interface SituationData { situation: string; nombre: number; }
interface SecteurData { secteur: string; offres: number; candidatures: number; }
interface Entonnoir {
  totalCandidatures: number;
  vues: number;
  entretiens: number;
...
interface StatsAvancees {
  annee: number;
  candidaturesParMois: MoisData[];
  inscriptionsParMois: MoisData[];
  parFiliere: FiliereData[];
  parNiveau: NiveauData[];
  parTypeOffre: TypeOffreData[];
  parStatutCandidature: StatutData[];
  parSituation: SituationData[];
  topSecteurs: SecteurData[];
  entonnoir: Entonnoir;
}
...
export default function DashboardAdmin() {
  ...
  const [avancees, setAvancees] = useState<StatsAvancees | null>(null);
  ...
  const chargerAvancees = useCallback(async () => {
    try {
            ...
            {activeTab === 'synthese' && (
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[
                    { to: '/admin/declarations', icon: 'fa-solid fa-clipboard-check', title: 'Valider déclarations', desc: 'Valider ou signaler les déclarations douteuses des diplômés', badge: 0, badgeColor: '', borderHover: 'hover:border-red-300' },
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                        ...
                        {avancees.parNiveau.length === 0 ? (
                          ...
                          <ResponsiveContainer width="100%" height={220}>
                            <PieChart>
                              <Pie data={avancees.parNiveau.map(r => ({ name: r.niveau, value: r.nombre }))}
                                cx="50%" cy="50%" outerRadius={80} dataKey="value"
                                label={({ name, value }) => `${name} (${value})`}>
                                {avancees.parNiveau.map((_: unknown, i: number) => (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                        ...
                        {avancees.parTypeOffre.length === 0 ? (
                          ...
                          <ResponsiveContainer width="100%" height={220}>
                            <BarChart data={avancees.parTypeOffre.map(r => ({ name: r.type.toUpperCase(), value: r.nombre }))}
                              ...
                              <Bar dataKey="value" name="Offres" radius={[6, 6, 0, 0]}>
                                {avancees.parTypeOffre.map((_: unknown, i: number) => (
                                  <Cell key={i} fill={COLORS[i % COLORS.length]} />
                                ))}
                              </Bar>
                            </BarChart>
                      ...
                      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                        ...
                        {avancees.parSituation.length === 0 ? (
                          ...
                          <div className="space-y-3">
                            {avancees.parSituation.map((s, i) => {
                              const total = avancees.parSituation.reduce((acc, x) => acc + x.nombre, 0);
